...
#modified term GO:0005515 protein binding 
...
modifygenesetsfunction <- function(genesets) {
  ...
  genesets$ontologies <- NA
  genesets$ontologies[substr(genesets$term, 1, 1) == "F"] <- "Molecular function"
  genesets$ontologies[substr(genesets$term, 1, 1) == "C"] <- "Cellular component"
  genesets$ontologies[substr(genesets$term, 1, 1) == "P"] <- "Biological process"
  genesets$`GO term` <- substr(genesets$term, 3, 12)
  genesets <- genesets[complete.cases(genesets$ontologies), ]
  return(genesets)
}

modifygenesets <- modifygenesetsfunction(genesets)

umgs <- unique(modifygenesets$`GO term`)
