...
```{r, stringsAsFactors=FALSE}
coldata <- read.table('sample_info.tsv', header = TRUE)
...
colData <- colData [ ,-1] 
colnames(colData) <- c("sampleid", "treatment", "stage", "timepoint", "group")
...
DBres <- DBresult(tca, group1 = "0_hpi", group2 = c("18_hpi","30_hpi","48_hpi"))
str(DBres, strict.width = "cut")
```

```{r}
head(DBres$`18-hpivs0-hpi`)
DBres.sig <- DBresult(tca, group1 = "0_hpi", group2 = c("18_hpi","30_hpi","48_hpi"), top.sig = TRUE)
...
#7295 genes clustered into similar gene expression patterns
...
p[[1]][["labels"]][["title"]] <- paste0("Cluster 1 - 1005 genes")
p[[2]][["labels"]][["title"]] <- paste0("Cluster 2 - 858 genes")
p[[3]][["labels"]][["title"]] <- paste0("Cluster 3 - 650 genes")
p[[4]][["labels"]][["title"]] <- paste0("Cluster 4 - 2082 genes")
p[[5]][["labels"]][["title"]] <- paste0("Cluster 5 - 1789 genes")
p[[6]][["labels"]][["title"]] <- paste0("Cluster 6 - 1460 genes")
```
...
```{r}
write.csv(cluster_6_genes.id, file = "/mnt/data/barry/projects/Schroeter_Pcinnamomi/shared_scripts/cluster6.tsv", row.names = FALSE)
```


```{r, warning=FALSE}

detectedgenes.id <- read_delim("detectedgenes_id.tsv", show_col_types = FALSE)
...
#How many predicted genes are common with effectorP and that share homology to those in in PHI_database?

```{r}
#import effector table containing both PHI blast and effectorP results 
effectors <- read_delim("effectortable.tsv", show_col_types = FALSE)

Cluster_labels <- effectors %>% 
  dplyr::select(c(gene_id, protein_id, product, Prediction, secreted, subject_id))

Cluster_labels <- Cluster_labels %>%
  mutate(PHI_homology = ifelse(subject_id != "NA", 1, 0),
         effectorP = ifelse(Prediction != "-" & Prediction != "Non-effector", 1, 0))

phipos <- Cluster_labels[Cluster_labels$PHI_homology == 1, ]
phipos <- unique(phipos$gene_id)

effectorppos <- Cluster_labels[Cluster_labels$effectorP == 1, ]
effectorppos <- unique(effectorppos$gene_id)

eprediction <- list("EffectorP"=effectorppos, "PHI_database"=phipos)

png(filename = "../Figures/predictionoverlap.png", width = 3600, height = 3600, res = 600)

plot(euler(eprediction),quantities = TRUE, labels = list(font = 24), main="prediction")

dev.off()
```

```{r}
countPredictions <- function(data) {
  total_rows <- nrow(data)
  effector_rows <- sum(data$Prediction %in% c("Apoplastic effector", "Cytoplasmic effector", "Apoplastic/cytoplasmic effector"))
  apoplastic_effector_rows <- sum(data$Prediction %in% c("Apoplastic effector", "Apoplastic/cytoplasmic effector"))
  cytoplasmic_effector_rows <- sum(data$Prediction %in% c("Cytoplasmic effector", "Apoplastic/cytoplasmic effector"))
  unique_gene_ids <- length(unique(data$gene_id))
  proportion_effector <- (effector_rows / total_rows) * 100
  proportion_apoplastic_effector <- (apoplastic_effector_rows / effector_rows) * 100
  proportion_cytoplasmic_effector <- (cytoplasmic_effector_rows / effector_rows) * 100
  
  cat("Total number of proteins:", total_rows, "\n")
  cat("Number of genes:", unique_gene_ids, "\n")
  cat("Number Effectors:", effector_rows, "\n")
  cat("Proportion of effectors:", proportion_effector, "%\n")
  cat("Proportion of Apoplastic effectors:", proportion_apoplastic_effector, "%\n")
  cat("Proportion of Cytoplasmic effectors:", proportion_cytoplasmic_effector, "%\n")
}

countPredictions(cluster_1_genes.id)
countPredictions(cluster_2_genes.id)
countPredictions(cluster_3_genes.id)
countPredictions(cluster_4_genes.id)
countPredictions(cluster_5_genes.id)
countPredictions(cluster_6_genes.id)
```
