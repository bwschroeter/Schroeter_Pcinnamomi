...
```{r, Early summary}
res_early_vs_vegetative <- results(dds, alpha=0.05, contrast = c("condition", "18h", "0h"))
...
png(filename = "../Figures/Early_pheatmap-scaledl2fc.png", width = 3600, height = 6400, res = 600)
...
ggplot(evol_df, aes(x = log2FoldChange, y = -log10(padj))) +
  ...
  geom_text(data = subset(etop_genes, baseMean > 1000), aes(label = Rank),
...
res_middle_vs_vegetative <- results(dds, alpha=0.05, contrast =c ("condition", "30h", "0h"))
...
png(filename = "../Figures/Mid_pheatmap-scaledl2fc.png", width = 4800, height = 6400, res = 600)

deph2 <- pheatmap(log2_expression_mid,
         scale = "row",
         cluster_rows = TRUE,
         treeheight_row = 0,
         fontsize_number = 10, 
...
res_late_vs_vegetative <- results(dds, alpha=0.05, contrast = c("condition", "48h", "0h"))
...
png(filename = "../Figures/Late_pheatmap-scaledl2fc.png", width = 4800, height = 6400, res = 600)
