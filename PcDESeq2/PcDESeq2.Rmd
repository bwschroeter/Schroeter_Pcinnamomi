...
earlysecreted <- dplyr::right_join(earlydetected, effectorp.results.id, by = 'gene_id')

ecyto <- subset(earlysecreted, Prediction %in% "Cytoplasmic effector")
eapo <- subset(earlysecreted, Prediction %in% "Apoplastic effector")
eeffector <- subset(earlysecreted, !Prediction %in% "Non-effector")
...
res_earlyearly_secreted <- earlysecreted[order(earlysecreted$pvalue), ]

sig <- subset(res_early_vs_vegetative, padj < 0.05)

eeup <- rownames(subset(eeffector, log2FoldChange > 2 & padj < 0.05))
eedn <- rownames(subset(eeffector, log2FoldChange < 2 & padj < 0.05))
