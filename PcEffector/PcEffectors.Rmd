...
png("predictedsecretome.png", width = 10, height = 6, units = "in", res = 300)
...
earlydesig$significant[earlydesig$log2FoldChange > 2 & earlydesig$padj < 0.05 | earlydesig$log2FoldChange < -2 & earlydesig$padj < 0.05] <- "*"
middesig$significant[middesig$log2FoldChange > 2 & middesig$padj < 0.05 | middesig$log2FoldChange < -2 & middesig$padj < 0.05] <- "*"
latedesig$significant[latedesig$log2FoldChange > 2 & latedesig$padj < 0.05 | latedesig$log2FoldChange < -2 & latedesig$padj < 0.05] <- "*"
```
...
eeup <- earlysecreted[earlysecreted$log2FoldChange > 2 & earlysecreted$padj < 0.05, ]
eeup <- unique(eeup$gene_id)
eedn <- earlysecreted[earlysecreted$log2FoldChange < -2 & earlysecreted$padj < 0.05, ]
eedn <- unique(eedn$gene_id)

meup <- midsecreted[midsecreted$log2FoldChange > 2 & midsecreted$padj < 0.05, ]
meup <- unique(meup$gene_id)
medn <- midsecreted[midsecreted$log2FoldChange < -2 & midsecreted$padj < 0.05, ]
medn <- unique(medn$gene_id)

leup <- latesecreted[latesecreted$log2FoldChange > 2 & latesecreted$padj < 0.05, ]
leup <- unique(leup$gene_id)
ledn <- latesecreted[latesecreted$log2FoldChange < -2 & latesecreted$padj < 0.05, ]
ledn <- unique(ledn$gene_id)

png("Secretomeexpressionstringent.png", width = 10, height = 6, units = "in", res = 300)

secreted <- list("Early_up"=eeup, "Early_down"=eedn, "Mid_up"=meup, "Mid_down"=medn, "Late_up"=leup,"Late_down"=ledn)

plot(euler(secreted),quantities = TRUE, labels = list(font = 24), main="In Planta predicted secretome expression")

dev.off()
```

```{r, secretome expression stats, echo=FALSE}
teeup <- length(earlysecreted[earlysecreted$log2FoldChange > 0 & earlysecreted$padj < 0.05, ])
teedn <- length(earlysecreted[earlysecreted$log2FoldChange < -0 & earlysecreted$padj < 0.05, ])

tmeup <- length(midsecreted[midsecreted$log2FoldChange > 0 & midsecreted$padj < 0.05, ])
tmedn <- length(midsecreted[midsecreted$log2FoldChange < 0 & midsecreted$padj < 0.05, ])

tleup <- length(latesecreted[latesecreted$log2FoldChange > 0 & latesecreted$padj < 0.05, ])
tledn <- length(latesecreted[latesecreted$log2FoldChange < 0 & latesecreted$padj < 0.05, ])
...
IUM83_07793 putative haustorium associated protein

```{r}
selected_rows <- dds_gene_expression[c("IUM83_07793", "IUM83_10648", "IUM83_06578"), ]
```

```{r}
Casgoi <- c("IUM83_07793", "IUM83_10648", "IUM83_06578")

Casgoiede <- earlydetected[earlydetected$gene_id %in% Casgoi, ]
Casgoimde <- middetected[middetected$gene_id %in% Casgoi, ]
Casgoilde <- latedetected[latedetected$gene_id %in% Casgoi, ]

goi_matrix <- matrix(
  c(Casgoiede$log2FoldChange, Casgoimde$log2FoldChange, Casgoilde$log2FoldChange),
  nrow = length(Casgoiede$log2FoldChange),
  ncol = 3,
  byrow = FALSE
)

rownames(goi_matrix) <- Casgoiede$gene_id
colnames(goi_matrix) <- c("Early", "Middle", "Late")

filename <- 
png("goi_Log2Fold_numbers.png", width = 3600, height = 6400, res = 600)
...
filename <- 
png("RxLR_Log2Fold_numbers.png", width = 3600, height = 6400, res = 600)
...
png("CRN_Log2Fold_numbers.png", width = 3600, height = 6400, res = 600)
...
png("NLP_Log2Fold_numbers.png",  width = 3600, height = 6400, res = 600)
...
png("Elicitin_Log2Fold_numbers.png", width = 3600, height = 6400, res = 600)
...
png("Secreted_enrichment_scores.png", width = 3600, height = 2400, pointsize = 10, res = 600)
...
png("Earlysecretedplot.png", width = 10, height = 10, units = "in", res = 300)
...
png("Midsecretedplot.png", width = 10, height = 10, units = "in", res = 300)
ggplot(res_df, aes(x = log10(baseMean), y = log2FoldChange)) +
  geom_point(color = "gray", alpha = 0.5, size = 2, shape = 19) +
...
png("Latesecretedplot.png", width = 10, height = 10, units = "in", res = 300)
ggplot(res_df, aes(x = log10(baseMean), y = log2FoldChange)) +
  geom_point(color = "gray", alpha = 0.5, size = 2, shape = 19) +
  geom_point(data = mssig, aes(color = "Effector"), size = 2, shape = 19) +
  geom_point(data = mesig, aes(color = "Secreted"), size = 2, shape = 19) +
